...
grid.style.display="grid";
  ...
  let isTesting       = false;
  let isChoukaiMode   = false;  // flag: sedang sesi Choukai aktif
...
// ==========================================
  ...
  function renderChoukai() {
    ...
    const slowBtn = isN5
      ? `<button class="choukai-btn-slow" id="choukaiSlowBtn" title="Putar lebih lambat">
           <span class="choukai-slow-icon">${CHOUKAI_SVG.clock}</span> Lambat
         </button>`
      : "";
  ...
  function _refreshQuizLayout() {
    ...
    // Re-render sesuai mode aktif — tanpa reset state apapun
...
// ==== LOGIKA KARTU KOSAKATA & PAGINATION ====
  ...
  async function saveScoreToCloud(type, level, correct, total, percentage) {
    ...
    try {
      ...
      await window.setDoc(docRef, {
        ...
        timestamp:  ts   // numerik untuk sorting yang benar
      });
    } catch (err) {
      ...
      ? historyItems.map((d) => {
          ...
          return `
            <article class="dashboard-history-item">
              <div>
                <p class="dashboard-history-date">${SVG_DASH.cal} ${d.tanggal ? d.tanggal.split(',')[0] : '-'}</p>
  ...
  async function fetchHistoryData(uid) {
    ...
    // Sertakan doc ID (yang merupakan timestamp ms) sebagai _docId untuk fallback sorting
    const items = [];
