...
// ==========================================
  ...
  function renderFavoritPage() {
    ...
    header.innerHTML = `
      <div class="favorit-header-left">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="#ff4d6d" stroke="#ff4d6d" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/>
        </svg>
        <span class="favorit-title">Favorit</span>
        <span class="favorit-counter" id="favoritPageCounter">0 kata</span>
      </div>
      <button class="favorit-clear-btn" id="favoritClearBtn" onclick="clearAllBookmarks()">Hapus Semua</button>
    `;
    grid.appendChild(header);

    // Ambil data kata yang di-bookmark
    if (typeof vocabularyData === "undefined" || userBookmarks.size === 0) {
      const empty = document.createElement("div");
      empty.className = "favorit-empty";
      empty.innerHTML = `
    ...
    const bookmarkedWords = vocabularyData.filter(w => {
      const id = String(w.id || w.kanji || w.kana || "");
      return userBookmarks.has(id);
    });
    ...
    // Update counter
    const counter = document.getElementById("favoritPageCounter");
    if (counter) counter.textContent = `${bookmarkedWords.length} kata`;

    // Grid mini cards
    const miniGrid = document.createElement("div");
    miniGrid.className = "favorit-mini-grid";

    bookmarkedWords.forEach(word => {
      const wordId = String(word.id || word.kanji || word.kana || "");
      const cardWrap = document.createElement("div");
      cardWrap.className = "favorit-mini-card";
      cardWrap.dataset.wordId = wordId;
      cardWrap.innerHTML = `
        <div class="favorit-mini-inner">
          ...
          <button class="favorit-mini-remove" onclick="removeFavoritCard(event, '${wordId}')" title="Hapus dari favorit">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="#ff4d6d" stroke="#ff4d6d" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/>
            </svg>
          </button>
        </div>
      `;
      // Klik mini card → expand modal dengan rekomendasi dari sesama bookmark
      cardWrap.addEventListener("click", (e) => {
        if (e.target.closest(".favorit-mini-remove")) return;
        openModalWithBookmarkRec(word, bookmarkedWords);
      });
      miniGrid.appendChild(cardWrap);
    });

    grid.appendChild(miniGrid);
  }

  // Buka modal, rekomendasi dari sesama bookmark
  function openModalWithBookmarkRec(word, bookmarkedWords) {
    const recs = bookmarkedWords.filter(w => {
      const id = String(w.id || w.kanji || w.kana || "");
      return id !== String(word.id || word.kanji || word.kana || "");
    }).slice(0, 6);
    openModal(word, recs);
  }

  // Hapus satu dari favorit via halaman favorit
  window.removeFavoritCard = function(event, wordId) {
    event.stopPropagation();
    userBookmarks.delete(String(wordId));
    if (window.currentUser) saveUserBookmarks(window.currentUser.uid);
    refreshBookmarkStateOnCards();
    // Re-render halaman favorit
    renderFavoritPage();
  };
